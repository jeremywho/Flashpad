...
- [x] **iOS: Swipe delete button flashes on swipe start** - Delete button is fully visible initially, then gets covered and reveals as expected during swipe (fixed: added opacity animation based on drag value)
- [x] **Mobile: Trash folder doesn't show newly deleted items** - Items moved to trash don't appear until pull-to-refresh (fixed: explicitly pass new tab/category values to fetchNotes to avoid stale closure issues)
